...
router.get('/input_realtime', async (req, res) => {
  try {
    const alarms = await status_realtime.find(); // ดึงข้อมูลทั้งหมดจาก collection
    res.json(alarms);
  } catch (err) {
...
router.post('/input_realtime', async (req, res) => {
  try {
    const { data } = req.body;
    console.log(data);
    const newAlarm = new status_realtime(data); // สร้างเอกสารใหม่จากข้อมูลที่ได้รับใน req.body
